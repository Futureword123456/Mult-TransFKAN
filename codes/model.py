...
class TemporalConvNet(nn.Module):
    ...
    def forward(self, x):
        x = x.transpose(1,
                        2)  # 将 batch_size, sequence_length, num_features 转换为 batch_size, num_features, sequence_length
        x = self.network(x)
        x = x[:, :, -1]  # 选择每个序列的最后一个输出
        x = self.fc(x)
        return x
...
#     def __init__(self, input_dim, num_heads, num_layers, num_outputs, hidden_space, dropout_rate=0.1, device='cuda'):
...
class PositionalEncoding(nn.Module):
    def __init__(self, hidden_space, max_len=5000):
        super(PositionalEncoding, self).__init__()
        self.hidden_space = hidden_space

        # 创建一个 (max_len, hidden_space) 的位置编码矩阵
        pe = torch.zeros(max_len, hidden_space)
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, hidden_space, 2).float() * (-math.log(10000.0) / hidden_space))
...
class TimeSeriesTransformer_ekan(nn.Module):
    def __init__(self, input_dim, num_heads, num_layers, num_outputs, hidden_space, dropout_rate=0.1):
        ...
        # 位置编码
        self.pos_encoder = PositionalEncoding(hidden_space)
        ...
        # 将 Encoder 的输出通过一个全连接层转换为所需的输出维度
        self.e_kan = NaiveFourierKANLayer([hidden_space, 10, num_outputs])
        self.transform_layer = nn.Linear(input_dim, hidden_space)
